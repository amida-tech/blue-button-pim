...
function compute(inputObj, outputObj, configJSON){
	...
	for(var key in inputObj){
		if(outputObj.hasOwnProperty (key)){
			...
			var weightValue = commonAlgos.absoluteStringMatch(inputObj[key], outputObj[key]);
...
//maximum score.
function findMaxWeights(inputObj, outputObj, configJSON){


	//compare two objects, object one and object two against each other.

	Object.keys(myJsonObject).length;
